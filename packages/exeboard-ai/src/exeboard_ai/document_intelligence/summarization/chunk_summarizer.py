...
def summarize_chunk(
    *,
    ...
    for generated_claim in generated_summary.claims:
        assembled_claim = _assemble_claim(
            chunk=chunk,
            document_type=document_type,
            chunk_context=chunk_context,
...
def _assemble_claim(
    *,
    ...
    for generated_evidence in generated_claim.evidence:
        assembled_evidence = _assemble_evidence(
            chunk=chunk,
            chunk_context=chunk_context,
...
def _get_cited_allowed_spans(
    *,
    ...
    unknown_span_ids = tuple(
        span_id for span_id in source_span_ids if span_id not in chunk_context.allowed_span_ids
    )
    if unknown_span_ids:
        raise ValueError("evidence source_span_ids must belong to allowed chunk context")
